...
\text{subject to:}&   &  &  & &\\
	&	&	& 	&	& \\
	 \displaystyle\sum\limits_{b \in B}\sum\limits_{j \in N}x_{ij}^{b} &=& 1 &  \forall i \in M & & (1) \\ %(i=1 ,..., m)% 
...
\SetKwInOut{Input}{Input}\SetKwInOut{Output}{Output}
\SetKw{KwBy}{by}
\Input{$M$ a dictionary of predefined active pick up stations and respective timeslots, $S$ a set of }
\Output{Drivers assignments for each timeslot}
\BlankLine
 initialization\;
n\_b $\leftarrow$ $len(M)$\;
n\_s $\leftarrow$ $len(S)$\;
\tcp{Define a 3 index variable that is binary and equals to 1 if the driver in this index is assigned this specific edge}
...
\begin{center}
 ...
 %\caption{Problem solution environment}\\
 \hline
