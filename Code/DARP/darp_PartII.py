...
model.addConstrs((sum(x[1, :, l]) == 1 for l in range(num_shifts)), name="c2")  # départ du dépot
model.addConstrs((sum(x[:, 2*num_bookings+1, l]) == 1 for l in range(num_shifts)), name="c3")
...
model.addConstrs((r[i-1] == u[num_bookings+i-1] - u[i-1] - 60 for i in range(1, num_bookings+1)), name="c9")
...
for i in range(2*num_bookings+2):
    if 1 <= i <= num_bookings:
        ...
        begin_date = data_bookings[i-1-num_bookings]["jobs"]["timewindow"][1] - 30
        end_date = begin_date + 30
...
for i in range(1,2*num_bookings+2):
